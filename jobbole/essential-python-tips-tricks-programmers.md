...
我们从开始使用 Python 便收集这些有用的捷径（贴士与技巧），有什么比分享我们知道的并同时有利于其他人的事还好的呢？
...
过去我们分享过[一些给初学者的 Python 编程贴士]()来优化代码并且减少编码工作，我们的读者仍旧很乐意阅读它。
...
所以今天我们带来另一些基本的 Python 贴士与技巧，所有的这些介壳都能帮助你压缩代码并且优化运行。此外，你可以在做一些正常任务时很容易地在真实项目中使用他们【*】。
...
Python 提供了一个直观的在一行代码中赋值与交换值得方法，请参见下面的示例：
...
赋值的右侧形成了一个新的元组，左侧立即解析那个（未被引用的）元组到变量 `<a>` 和 `<b>`。
...
这里给出几个你可以用来使代码紧凑简洁的例子。下面的语句是说“如有 y 是 9，给 x 赋值 10，不然的话给 x 赋值为 20”。如果需要的话我们也可以延长这条操作链。
...
上面方法共有的问题是缺少缩进，我们我们尝试缩进会在字符串中插入空格。所以最后的解决方案是将字符串分为多行并且将整个字符串包含在括号中：
...
我们可以使用列表来初始化多个变量，在解析列表时，变量的数目不应该超过列表中的元素个数：【译者注，元素个数与列表长度应该相同】【*】
...
这是一个许多人不知道的有用特性，在 Python 控制台，不论何时我们测试一个表达式或者调用一个方法，结果都会分配给一个临时变量， `_`（一个下划线）。
...
与我们使用的列表推导相似，我们也可以使用字典/集合推导，它们使用简单并且有效，下面是一个例子：
...
上面的命令会在默认端口也就是 8000 开启一个服务器，你可以将一个自定义的端口号以最后一个参数传递到上面的命令中。
...
对于 `in` 操作符我们可以使用 ‘{1,3,5,7}’ 而不是 ‘[1,3,5,7]’，因为 `set` 中取元素是 O(1) 操作。
...
没有太多编程语言支持这个特性，然而 Python 中的方法确实（可以）返回多个值，请参见下面的例子来看看是如何工作的：
...
在 Python 2.7 中，一个 32 比特的整数占用 24 byte，在 Python 3.5 中利用 28 byte。为确定内存使用，我们可以调用 `getsizeof` 方法：
...
【译者注：在我的 windows 上 python2.7 中上面的结果是
```
#In Python 2.7
#1-> 896
#2-> 1016
...
所以，这种比较方式是不那么让人信服的，使用 `__slots__` 主要是用以规定对象的属性信息，另外，当生成对象很多时花销可能会小一些，具体可以参见 [python 官方文档](https://docs.python.org/3/reference/datamodel.html?highlight=__slots__#object.__slots__)
】
